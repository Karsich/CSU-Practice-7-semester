...
- **RTSP потоки**: Используются для обработки через OpenCV в **HD качестве** (`quality=hd`)
- **HLS потоки**: Доступны для прямого просмотра в браузере
- **Частота обработки**: ~10 FPS (каждый 3-й кадр обрабатывается)
- **Формат кадров**: JPEG, качество 90% (повышено для HD кадров)
- **Детекция**: YOLOv8n для распознавания людей, автобусов и автомобилей
- **Разрешение детекции**: Автоматически адаптируется для HD кадров (1280px для HD, 640px для стандартных)
- **OCR распознавание**: Оптимизировано для HD кадров с улучшенной обработкой изображений
...
- При недоступности HD потока система автоматически переключается на стандартное качество
...
- HD качество улучшает точность распознавания номеров автобусов и детекции объектов
